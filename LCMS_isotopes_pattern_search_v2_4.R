...
# Fixed after R update on 7/02/2024
...
eicwidth<-0.001 #m/z width of extracted ion chromatogram (+/- Da)
...
Isotope_pipeline <- function(isotopepattern,mzxcms,timerange,background,peakwidth,savefile){
  ...
  if(length(results)>2){
    ...
    if(nrow(cleanresults)>0){
      ...
      text(as.numeric(epattern$diff),as.numeric(epattern$rat),labels=c(epattern$element),cex=0.6,pos=3,col='blue')
